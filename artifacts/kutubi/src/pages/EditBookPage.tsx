...
export default function EditBookPage() {
  ...
  useEffect(() => {
    if (book && !initialized) {
      ...
      setPrice(book.is_free ? '' : String(book.price));
      setIsFree(book.is_free);
      setDescription(book.description || '');
      setGovernorate(book.governorate);
      setCity(book.city);
      setDelivery(book.delivery);
  ...
  if (isLoading) {
    return <div className="flex-1 flex items-center justify-center"><div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin"></div></div>;
  }
  ...
  if (!user || user.id !== book.seller_id) {
    return <div className="p-8 text-center text-xl text-destructive">غير مسموح لك بتعديل هذا الكتاب</div>;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateBook.mutateAsync({
        ...
        updates: {
          title: title.trim(),
          subject, grade, track, publisher, condition,
          price: isFree ? 0 : numericPrice,
          is_free: isFree,
          image_url: imageUrl,
          description: description.trim(),
          governorate, city, delivery,
        }
      });
  ...
  if (success) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in">
        ...
        <h2 className="text-3xl font-bold text-foreground mb-4">تم تحديث الكتاب بنجاح!</h2>
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <div className="flex items-center gap-3 mb-8">
        <Link href="/dashboard" className="text-muted-foreground hover:text-primary transition-colors flex items-center gap-1 text-sm">
          <ArrowRight className="w-4 h-4" />
          لوحة التحكم
        </Link>
        <span className="text-muted-foreground">/</span>
        <span className="font-semibold text-foreground">تعديل الكتاب</span>
      </div>

      <h1 className="text-3xl font-bold text-foreground mb-8">تعديل بيانات الكتاب</h1>

      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-8">
        {error && (
          <div className="bg-destructive/10 text-destructive p-4 rounded-xl text-sm font-bold">{error}</div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold">عنوان الكتاب <span className="text-destructive">*</span></label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold flex justify-between">
                ...
                <label className="flex items-center gap-1.5 cursor-pointer text-primary">
                  <input type="checkbox" checked={isFree} onChange={e => { setIsFree(e.target.checked); if (e.target.checked) setPrice('0'); else setPrice(''); }} className="accent-primary w-4 h-4" />
                  مجاني لوجه الله
                </label>
              </label>
              <input type="number" min="0" max="100" required={!isFree} disabled={isFree}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold">صورة الغلاف</label>
            <div onClick={() => fileInputRef.current?.click()}
              ...
              <input type="file" ref={fileInputRef} onChange={handleImageSelect} accept="image/jpeg,image/png,image/webp" className="hidden" />
              {imagePreview ? (
                <>
                  <img src={imagePreview} className="absolute inset-0 w-full h-full object-contain bg-black/5" />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white">تغيير الصورة</div>
                </>
              ) : (
                <>
                  <div className="bg-primary/10 p-4 rounded-full mb-3 text-primary"><ImageIcon className="w-8 h-8" /></div>
          ...
          <div className="space-y-2">
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)}
              ...
              className="w-full bg-background border border-input rounded-xl px-4 py-3 outline-none focus:border-primary focus:ring-1 min-h-[100px] resize-y"></textarea>
          </div>
        </div>
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer p-3 border border-border rounded-xl flex-1 hover:bg-muted bg-background">
                  ...
                  <div className="flex flex-col"><span className="font-bold text-sm">مقابلة شخصية</span><span className="text-xs text-muted-foreground">المشتري يجي يستلم</span></div>
                </label>
                <label className="flex items-center gap-2 cursor-pointer p-3 border border-border rounded-xl flex-1 hover:bg-muted bg-background">
                  <input type="radio" name="delivery" checked={delivery === 'shipping'} onChange={() => setDelivery('shipping')} className="accent-primary w-4 h-4" />
                  <div className="flex flex-col"><span className="font-bold text-sm">متاح شحن</span><span className="text-xs text-muted-foreground">عن طريق البريد مثلاً</span></div>
                </label>
              </div>
            </div>
          </div>
        ...
        <div className="pt-4 flex gap-3">
          <Link href="/dashboard" className="flex-1 text-center border border-border text-foreground font-bold py-4 rounded-xl transition-colors hover:bg-muted">
            إلغاء
          </Link>
          <button type="submit" disabled={isUploading}
            className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2 disabled:opacity-70 shadow-md">
            {isUploading ? (
              <><Loader2 className="w-5 h-5 animate-spin" /> جاري الحفظ...</>
            ) : (
              <><Upload className="w-5 h-5" /> حفظ التعديلات</>
            )}
          </button>
        </div>
      </form>
